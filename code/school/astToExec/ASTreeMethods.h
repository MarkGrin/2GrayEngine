...
void ASTree :: toExec (ASTNode* node)
{
    ...
    else if ( code == STANDARD_FUNC )
    {
        ...
        {
            printf ("STD_FUNC call son: value:%d code:%f\n", node->getChild(0)->getValue(), node->getChild(0)->getCode());
            printf ("STD_FUNC call    : value:%d code:%f\n", node->getValue(), node->getCode());
            printf ("getting jump of %f is %d\n", node->getChild(0)->getValue(), funcJumps_[node->getChild(0)->getValue()]);
